...
- MACアドレス: Ethernetパケットの送信先アドレス。IPパケットでないことに注意。
- ARP(Address Resplution Protocol): IPアドレス/MACアドレス対応調査。
...
- Ethernetはハードウェア規格だけでなく、パケット形式やアドレスを規定する通信規約でもある。
- EthernetではMACアドレスというIPアドレスとは異なる独自アドレスを用いる。
- MACアドレスはネットワーク接続用インターフェイス(NIC)に製造元が刻印した番号。設定で変更するようなものではない。
- NICを変更すれば、MACアドレスも変わる。
- MACアドレスはアドレスというより製造番号に近い。しかし、どの製品も同じ形式で、重複しないよう統一管理されている。
- MACアドレスは48ビット長。扱いやすいよう、6つの8ビットに分け、-で区切り16進数で表記することが多い。(Macだと:区切り?)
- MACアドレスは特殊用途アドレスでARPで用いる全部1のブロードキャストアドレスなどがある。リミテッドブロードキャストと機能的には同様。
- ネットワークにデータを送信するとき、データを宛先IPアドレスを書き込んだIPパケットに格納する。
- IPパケットを物理搬送手段であるEthernetパケットに格納する。パケットがコンテナなら、Ethernetはトラックのようなもの。
- EthernetパケットもIPパケット同様、宛先、送信元アドレスなど通信制御情報などをヘッダに格納している。
- IPパケットもデータ領域とヘッダ領域があるため、入れ子のような状態になる。これをカプセル化という。
- Ethernetパケットは同じネットワークであれば、IPアドレスとMACアドレスは同じ相手をさす。
- 違うネットワークでルータを介す場合、まずルータがEthernetパケットを受け取りIPパケットを取り出し宛先IPアドレスをチェックする。
- その後、新たにEthernetパケットを作成し、IPパケットを乗せて送信される。
- そのため、MACアドレス的には送信元はルータだがIPアドレス的には送信元コンピュータなので、送信元から直接送られたように映る。
- ネットワーク内の通信ならIPアドレスとMACアドレスが同じ相手を指す必要があり、調べなければならない。
- ネットワーク外の通信ならデフォルトゲートウェイアドレスのMACアドレスを調べる必要がある。
- ARPを用いて、IPアドレス/MACアドレスの対応調査を行う。
- ARPは「このIPアドレスを持つコンピュータは自分のMACアドレスを知らせよ」と問いかけ、アドレス対応を調べる。
- これは、今電話している相手に電話番号を聞いているような状態である(通信してMACアドレスを聞いている)。
- そこで、アドレス対応を知りたいコンピュータは、IPアドレスを書き込んだEthernetパケットをブロードキャストアドレスにブロードキャストする。これをARP要求という。
- ARP要求はルータを含むネットワークの全コンピュータに送られる。書き込まれたIPアドレスと一致するものはARP応答を返す。
- ARP要求は全コンピュータのリソースを消費するため、通信効率を低下させる。
- そこでAPRの結果をデータベースに保存し、キャッシュする。これをARPキャッシュという。
- NIC交換などで変わることもあるので、一定期間参照されてない対応付けは破棄される。
- IPアドレスの重複検査はARPを利用している。
- IEEEなどで規格化されている。詳細はP42参照のこと。
